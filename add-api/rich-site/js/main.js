...
//
function appendPhotoTodiv(v_per_page,v_text,v_appendTo){
  var parameters =  $.param({
    method: "flickr.photos.search",
    api_key: apiKey,
    text: v_text, // 検索テキスト
    sort: "interestingness-desc", // 興味深さ順
    per_page: v_per_page, // 取得件数
  ...
  $.getJSON(flickr_url, function(data){
    ...
    if (data.stat == "ok") {
      ...
      for (var i = 0; i < data.photos.photo.length; i++) {
        ...
        var $div = $("<div>");
        $div.addClass("image-gallery__item");
        $div.append(
          $("<a>", {
        ...
        $div.appendTo($div_base);
      }
      // $divを#mainに追加する
      $div_base.appendTo(v_appendTo);
...
$(document).ready(function() {
  //gallery画像追加
  appendPhotoTodiv(4,"cat","#image-gallery-main");
  appendPhotoTodiv(4,"dog","#image-gallery-main");
